...
const Tasks = () => {
  ...
  const handleCreateTask = () => { setEditingTask(null); setShowTaskModal(true); };
  ...
  const handleTaskSuccess = () => { fetchTasks(); };
  ...
  const getStatusColor = (status) => {
    const colors = { pending: '#64748b', in_progress: '#2563eb', completed: '#10b981', on_hold: '#f59e0b' };
  ...
  return (
    <div className="tasks-page">
      ...
      {loading ? (
        <div className="tasks-list">{[1, 2, 3].map((i) => <CardSkeleton key={i} />)}</div>
      ) : filteredTasks.length === 0 ? (
        <EmptyState type="tasks" message={searchQuery || statusFilter !== 'all' ? 'No tasks match your search criteria.' : undefined} />
      ) : (
            ...
            <div key={task._id} className="task-card">
              <div className="task-header">
                ...
                <div className="task-header-actions">
                  <span className="status-badge" style={{ backgroundColor: getStatusColor(task.status) + '20', color: getStatusColor(task.status) }}>
                  ...
                  {(canEditTask || canDeleteTask) && (
                    <div className="task-actions">
                      ...
                      {canDeleteTask && (
                        <button className="btn-icon btn-delete" onClick={() => { setDeletingTask(task); setShowDeleteConfirm(true); }} title="Delete task">
                          <FiTrash2 />
                        </button>
                      )}
                    </div>
      ...
      <ConfirmDialog
        ...
        onClose={() => { setShowDeleteConfirm(false); setDeletingTask(null); }}
